...
class QuadraticEquationRoots(Evaluator):
    ...
    def _eval_standard(self):
        D = np.power(np.power(self.coeffs[1], 2.0, dtype=self.dtype) - self.dtype(4) * self.coeffs[0] * self.coeffs[2], (1/2), dtype=self.dtype)

        x1 = -(self.coeffs[1] - D) / (self.dtype(2) * self.coeffs[2])
        x2 = -(self.coeffs[1] + D) / (self.dtype(2) * self.coeffs[2])
