...
def tweak(colors, n_max_colors):
    ...
    tweak_node = np.random.randint(len(colors))
    tweak_color = np.random.randint(n_max_colors)

    while tweak_color == tweak_colors[tweak_node]:
        tweak_color = np.random.randint(n_max_colors)

    tweak_colors[tweak_node] = tweak_color
    return tweak_colors
...
def solve_via_hill_climbing(
    ...
    for i in range(n_iters):
        ...
        for _ in range(n_tweaks):
            next_colors = tweak(cur_colors, n_max_colors)
            n_conflicts = number_of_conflicts(G, next_colors)
            if n_conflicts < n_conflicts_best:
                next_colors_best = next_colors
                n_conflicts_best = n_conflicts
        if n_conflicts_best < n_conflicts:
            cur_colors = next_colors_best

    return loss_history


#бежим по колву итераций, на кажд генерируем случ вектор цветов и раскрашиваем граф (вызыываем number_of_conflicts), записываем в loss_history
def solve_via_random_search(
    ...
    for i in range(n_iters):
        colors = np.random.randint(0, n_max_colors - 1, len(G.nodes))
        loss_history.append(number_of_conflicts(G, colors))
